...
function addAssetToList() {
    ...
    for (var row in data.rows) {
        if (contains(assetIdList, data[index].id) == false)
...
function saveChangeManager(type) {
    $('#saveBtn').removeAttr('disabled');
    var oldAssetManagerId = $('#oldAssetManagerId').val();
    var assetManagerId = $('#assetManagerId').combobox('getValue');
    ...
    $.ajax({
        url: 'assetChange/save_change_manager',
        type: 'POST',
        data: {
            oldAssetManagerId: oldAssetManagerId,
        ...
        success: function (data) {
            ...
            if (type == 1) {
                $.messager.alert('提示', '保存成功！', 'info', function () {
                    window.location.href = 'asset_maneger_change_receiptList';
                });
            } else {
                var data1 = {};
                data1.id=data.id;
                data1.formName = "管理员变更申请单";
                data1.serialNumber = data.changeNum;
                data1.processDefKey = ASSETS_USER_LOCATION_CHANGE;
                data1.resultLocation = "asset_maneger_change_receiptList";
...
function saveAndCheckChangeManager() {
    ...
    var assetIdListStr = $('#assetList input[name="assetIdList"]').val();
    $.ajax({
        url: 'assetChange/save_and_check_change_manager',
        type: 'POST',
        data: {
            oldAssetManagerId: oldAssetManagerId,
            assetManagerId: assetManagerId,
...
function openAssetStanBookDialog() {
    ...
    var oldAssetManagerId = $('#oldAssetManagerId').val();
    if (oldAssetManagerId == "") {
        $.messager.alert('错误', '请先选择原资产管理员！', 'info');
    } else {
        defaultSearch();

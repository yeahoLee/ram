...
function lookReceipt(id) {
    window.location.href = './asset_manager_change_show?id=' + id;
}
...
function deleteManagerChangeReceipt(id) {
    $.messager.confirm('确定', '确定要删除吗？', function (r) {
        if (r) {
            $.ajax({
                url: 'assetChange/delete_change_manager_receipt',
                type: 'POST',
                data: {id: id},
...
function toUpdateManagerChangeReceipt(id) {
    window.location.href = './update_manager_change_receipt?id=' + id;
}
...
function searchByQuerys() {
    ...
    $('#receiptTable').datagrid('load', {
        ...
        assetManagerId: $('#assetManagerId').combobox("getValue"),
...
$(function () {
    ...
    $('#receiptTable').datagrid({
        url: 'assetChange/manager_change_receipt_datagrid',
        fit: true,
        method: 'POST',
        striped: true,
        rownumbers: true,
        ...
        columns: [[
            ...
            {field: 'oldAssetManagerStr', title: '原管理员'},
            {field: 'assetManagerStr', title: '新管理员'},

...
function saveChangeUser(type) {
    ...
    $.ajax({
        url: 'assetChange/save_change_user',
        type: 'POST',
        data: {
        ...
        success: function (data) {
            ...
            if (type == 1) {
                $.messager.alert('提示', '保存成功！', 'info', function () {
                    window.location.href = 'asset_user_change_list';
                });
            } else {
                var data1 = {};
                data1.id=data.id;
                data1.formName = "使用人变更申请单";
                data1.serialNumber = data.changeNum;
                data1.processDefKey = ASSETS_USER_LOCATION_CHANGE;
                data1.resultLocation = "asset_user_change_list";
...
function saveAndCheckChangeUser() {
    $('#saveandsendBtn').attr('disabled', "true");
    var assetUserId = $('#assetUserId').combobox('getValue');
    var reason = $('#reason').val();
    var assetIdListStr = $('#assetList input[name="assetIdList"]').val();
    $.ajax({
        url: 'assetChange/save_and_check_change_user',
        type: 'POST',
        data: {
            assetUserId: assetUserId,
            reason: reason,

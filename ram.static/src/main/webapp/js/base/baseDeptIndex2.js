...
function setDeptUserSubmit() {
    ...
    $.ajax({
        ...
        success: function () {
            ...
            $('#addUserToDeptDialog').dialog('close');
        },
        error: function (data) {
            AjaxErrorHandler(data);
        }
...
function openAddUserToDeptDialog() {
    $('#userId').combobox('clear');
    $('#addUserToDeptDialog').dialog('center').dialog('open');
}
...
$(function () {
    $('#dataGridTable').treegrid({
        ...
        columns: [[
            ...
            {field: 'text', title: '部门名称'},
            {field: 'code', title: '部门编码'},
            {field: 'pname', title: '上级部门名称'}
        ]]
    });
